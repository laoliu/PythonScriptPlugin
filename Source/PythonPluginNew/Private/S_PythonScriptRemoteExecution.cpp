...
public:
		...
		, BroadcastSocket(nullptr)
	{
		...
		const UPythonScriptPluginSettings_S* PythonScriptPluginSettings = GetDefault<UPythonScriptPluginSettings_S>();
...
private:
	...
	void HandleCommandMessage(const FPythonScriptRemoteExecutionMessage& InMessage)
	{
		...
		if (InMessage.Data && InMessage.Data->TryGetObject(CommandData) && *CommandData)
		{
...

			{
				...
				PythonCommand.Flags |= bUnattended ? EPythonCommandFlags_S::Unattended : EPythonCommandFlags_S::None;
			}
	...
	void SendCommandResultMessage(const FString& InDestinationNodeId, const bool bSuccess, const FString& InCommand, const FString& InResult, const TArray<FPythonLogOutputEntry_S>& InLogOutput)
	{
		TArray<TSharedPtr<FJsonValue>> JsonLogOutput;
		for (const FPythonLogOutputEntry_S& LogEntry : InLogOutput)
		{
	...
	, NodeId(FApp::GetInstanceId().ToString())
{
	...
	if (GetDefault<UPythonScriptPluginSettings_S>()->bRemoteExecution)
	{
		Start();
	}
}
