...
#include UE_INLINE_GENERATED_CPP_BY_NAME(S_PyWrapperEnum)
...
void FPyWrapperEnumMetaData::AddTypeReferencedObjects(FReferenceCollector& Collector)
{
...
public:
		...
		, NewEnum(nullptr)
	{
		...
		NewEnum = FindObject<UPythonGeneratedEnum_S>(EnumOuter, *EnumName);
		if (!NewEnum)
		{
			NewEnum = NewObject<UPythonGeneratedEnum_S>(EnumOuter, *EnumName, RF_Public | RF_Transient);
	...
	UPythonGeneratedEnum_S* Finalize(const TArray<FPyUValueDef*>& InPyValueDefs)
	{
		...
		UPythonGeneratedEnum_S* FinalizedEnum = NewEnum;
		NewEnum = nullptr;
	...
	bool CreateValueFromDefinition(const FString& InFieldName, FPyUValueDef* InPyValueDef)
	{
		...
		UPythonGeneratedEnum_S::FEnumValueDef& EnumValueDef = *NewEnum->EnumValueDefs.Add_GetRef(MakeShared<UPythonGeneratedEnum_S::FEnumValueDef>());
...
private:
	bool RegisterDescriptors(const TArray<FPyUValueDef*>& InPyValueDefs)
	{
		...
		NewEnum->EnumValueDefs.Sort([](const TSharedPtr<UPythonGeneratedEnum_S::FEnumValueDef>& EnumValueDefOne, const TSharedPtr<UPythonGeneratedEnum_S::FEnumValueDef>& EnumValueDefTwo)
		{
		...
		check(InPyValueDefs.Num() == NewEnum->EnumValueDefs.Num());
		{
			...
			for (const TSharedPtr<UPythonGeneratedEnum_S::FEnumValueDef>& EnumValueDef : NewEnum->EnumValueDefs)
			{
			...
			{
				TSharedPtr<UPythonGeneratedEnum_S::FEnumValueDef>& EnumValueDef = NewEnum->EnumValueDefs[EnumValueIndex];
	...
	FString EnumName;
	PyTypeObject* PyType;
	UPythonGeneratedEnum_S* NewEnum;
};

void UPythonGeneratedEnum_S::BeginDestroy()
{
	ReleasePythonResources();
	Super::BeginDestroy();
}

void UPythonGeneratedEnum_S::ReleasePythonResources()
{
...
void UPythonGeneratedEnum_S::UnregisterGeneratedType()
{
	if (PyType)
	{
...
UPythonGeneratedEnum_S* UPythonGeneratedEnum_S::GenerateEnum(PyTypeObject* InPyType)
{
