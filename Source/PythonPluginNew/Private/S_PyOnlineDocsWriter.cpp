...
{
	return FPaths::ConvertRelativePathToFull(FPaths::EnginePluginsDir()) / TEXT("PythonPluginNew/SphinxDocs");
}
...
void FPyOnlineDocsWriter::GenerateFiles(const FString& InPythonStubPath)
{
	...
	// Copy generated unreal module stub file to PythonPluginNew/SphinxDocs/modules
	{
	...
	if (bUseSphinx)
	{
		...
		// Run Sphinx to build the docs
		{
			...
			UE_LOG(LogPython, Display, TEXT(
				...
				"  PythonPluginNew/SphinxDocs/PythonAPI_docs_readme.txt\n"),
				*GetBuildPath());
		}
	}
	else
	{
		...
		UE_LOG(LogPython, Display, TEXT(
			...
			"  PythonPluginNew/SphinxDocs/PythonAPI_docs_readme.txt\n\n"
			"And then call:"
			"  PythonPluginNew/SphinxDocs/sphinx-build -b html source/ build/"));
	}
}
