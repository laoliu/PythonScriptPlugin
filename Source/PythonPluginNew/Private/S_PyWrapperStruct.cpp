...
#include UE_INLINE_GENERATED_CPP_BY_NAME(S_PyWrapperStruct)
...
public:
		...
		, NewStruct(nullptr)
	{
		...
		OldStruct = FindObject<UPythonGeneratedStruct_S>(StructOuter, *StructName);

		// Create a new struct with a temporary name; we will rename it as part of Finalize
		const FString NewStructName = MakeUniqueObjectName(StructOuter, UPythonGeneratedStruct_S::StaticClass(), *FString::Printf(TEXT("%s_NEWINST"), *StructName)).ToString();
		NewStruct = NewObject<UPythonGeneratedStruct_S>(StructOuter, *NewStructName, RF_Public | RF_Transient);
	...
	UPythonGeneratedStruct_S* Finalize(FPyObjectPtr InPyPostInitFunction)
	{
		...
		UPythonGeneratedStruct_S* FinalizedStruct = NewStruct;
		NewStruct = nullptr;
...
private:
	...
	FString StructName;
	PyTypeObject* PyType;
	UPythonGeneratedStruct_S* OldStruct;
	UPythonGeneratedStruct_S* NewStruct;
};

void UPythonGeneratedStruct_S::PostRename(UObject* OldOuter, const FName OldName)
{
...
void UPythonGeneratedStruct_S::InitializeStruct(void* Dest, int32 ArrayDim) const
{
	...
	// Execute Python code within this block
	{
		...
		if (PyPostInitFunction)
		{
			...
			for (int32 ArrIndex = 0; ArrIndex < ArrayDim; ++ArrIndex)
			{
				...
				FPyObjectPtr PySelf = FPyObjectPtr::StealReference((PyObject*)FPyWrapperStructFactory::Get().CreateInstance((UPythonGeneratedStruct_S*)this, StructInstance, FPyWrapperOwnerContext(Py_None), EPyConversionMethod::Reference));
...
void UPythonGeneratedStruct_S::BeginDestroy()
{
	ReleasePythonResources();
	Super::BeginDestroy();
}

void UPythonGeneratedStruct_S::ReleasePythonResources()
{
...
void UPythonGeneratedStruct_S::UnregisterGeneratedType()
{
	if (PyType)
	{
...
UPythonGeneratedStruct_S* UPythonGeneratedStruct_S::GenerateStruct(PyTypeObject* InPyType)
{
