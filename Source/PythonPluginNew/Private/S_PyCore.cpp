...
PyObject* Log(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (PyArg_ParseTuple(InArgs, "O:log", &PyObj))
	{
		...
		if (ShouldLog(LogMessage))
		{
			...
			GetPythonLogCapture().Broadcast(EPythonLogOutputType_S::Info, *LogMessage);
		}
...
PyObject* LogWarning(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (PyArg_ParseTuple(InArgs, "O:log_warning", &PyObj))
	{
		...
		if (ShouldLog(LogMessage))
		{
			...
			Py_END_ALLOW_THREADS
			GetPythonLogCapture().Broadcast(EPythonLogOutputType_S::Warning, *LogMessage);
		}
...
PyObject* LogError(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (PyArg_ParseTuple(InArgs, "O:log_error", &PyObj))
	{
		...
		if (ShouldLog(LogMessage))
		{
			...
			Py_END_ALLOW_THREADS
			GetPythonLogCapture().Broadcast(EPythonLogOutputType_S::Error, *LogMessage);
		}
...
PyObject* Reload(PyObject* InSelf, PyObject* InArgs)
{
	...
	FPythonPluginNew* PythonScriptPlugin = FPythonPluginNew::Get();
...
PyObject* GenerateClass(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (!FPyWrapperObjectMetaData::GetMetaData(PyType) && !UPythonGeneratedClass_S::GenerateClass(PyType))
	{
...
PyObject* GenerateStruct(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (!FPyWrapperStructMetaData::GetMetaData(PyType) && !UPythonGeneratedStruct_S::GenerateStruct(PyType))
	{
...
PyObject* GenerateEnum(PyObject* InSelf, PyObject* InArgs)
{
	...
	if (!FPyWrapperEnumMetaData::GetMetaData(PyType) && !UPythonGeneratedEnum_S::GenerateEnum(PyType))
	{
...
PyObject* CreatePythonObjectHandle(PyObject* InSelf, PyObject* InArgs)
{
	...
	UPythonObjectHandle_S* Handle = UPythonObjectHandle_S::Create(PyObj);
...
PyObject* RegisterPythonShutdownCallback(PyObject* InSelf, PyObject* InArgs)
{
	...
	FDelegateHandle PythonShutdownDelegateHandle = FPythonPluginNew::Get()->OnPythonShutdown().AddLambda([PyCallable = FPyAutoGILPtr(FPyObjectPtr::NewReference(PyObj))]() mutable
	{
...
PyObject* UnregisterPythonShutdownCallback(PyObject* InSelf, PyObject* InArgs)
{
	...
	{
		FPythonPluginNew::Get()->OnPythonShutdown().Remove(PythonEventDelegateHandle->Value);
	}
...
FPythonLogCapture_S& GetPythonLogCapture()
{
	static FPythonLogCapture_S PythonLogCapture;
