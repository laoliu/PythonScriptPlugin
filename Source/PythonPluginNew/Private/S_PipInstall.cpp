...
// In order to keep editor startup time fast, check directly for this utils version (make sure to match with wheel version in PythonPluginNew/Content/Python/Lib/wheels)
...
FString FPipInstall::ParsePluginDependencies(const FString& MergedInRequirementsFile, FFeedbackContext* Context) const
{
	...
	if (!GetDefault<UPythonScriptPluginSettings_S>()->bPipStrictHashCheck)
	{
...
FPipInstall::FPipInstall()
{
	...
	bRunOnStartup = GetDefault<UPythonScriptPluginSettings_S>()->bRunPipInstallOnStartup;
...
void FPipInstall::WriteSitePackagePthFile() const
{
	...
	// Additional paths
	for (const FDirectoryPath& AdditionalPath : GetDefault<UPythonScriptPluginSettings_S>()->AdditionalPaths)
	{
...
{
	const UPythonScriptPluginSettings_S* ScriptSettings = GetDefault<UPythonScriptPluginSettings_S>();
...
{
	TSharedPtr<IPlugin> PythonPlugin = IPluginManager::Get().FindPlugin("PythonPluginNew");
	if (!PythonPlugin)
	{
		return TEXT("");
	}
