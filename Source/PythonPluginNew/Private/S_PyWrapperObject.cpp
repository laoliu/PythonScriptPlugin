...
#include "S_PyWrapperOwnerContext.h"
...
#include UE_INLINE_GENERATED_CPP_BY_NAME(S_PyWrapperObject)
...
public:
		...
		, NewClass(nullptr)
	{
		...
		OldClass = FindObject<UPythonGeneratedClass_S>(ClassOuter, *ClassName);

		// Create a new class with a temporary name; we will rename it as part of Finalize
		const FString NewClassName = MakeUniqueObjectName(ClassOuter, UPythonGeneratedClass_S::StaticClass(), *FString::Printf(TEXT("%s_NEWINST"), *ClassName)).ToString();
		NewClass = NewObject<UPythonGeneratedClass_S>(ClassOuter, *NewClassName, RF_Public | RF_Transient);
	...
	FPythonGeneratedClassBuilder(UPythonGeneratedClass_S* InOldClass, UClass* InSuperClass)
	...
	UPythonGeneratedClass_S* Finalize(FPyObjectPtr InPyPostInitFunction)
	{
		...
		if (OldClass)
		{
			...
			UPythonGeneratedClass_S::ReparentDerivedClasses(OldClass, NewClass);
		}	
		...
		UPythonGeneratedClass_S* FinalizedClass = NewClass;
		NewClass = nullptr;
	...
	bool CreateFunctionFromDefinition(const FString& InFieldName, FPyUFunctionDef* InPyFuncDef)
	{
		...
		NewClass->AddNativeFunction(*FuncName.ToString(), &UPythonGeneratedClass_S::CallPythonFunction); // Need to do this before the call to DuplicateObject in the case that the super-function already has FUNC_Native
	...
	bool CopyFunctionsFromOldClass()
	{
		...
		for (const TSharedPtr<PyGenUtil::FFunctionDef>& OldFuncDef : OldClass->FunctionDefs)
		{
			...
			NewClass->AddNativeFunction(*OldFunc->GetName(), &UPythonGeneratedClass_S::CallPythonFunction);
...
private:
	...
	FString ClassName;
	PyTypeObject* PyType;
	UPythonGeneratedClass_S* OldClass;
	UPythonGeneratedClass_S* NewClass;
};

void UPythonGeneratedClass_S::PostRename(UObject* OldOuter, const FName OldName)
{
...
void UPythonGeneratedClass_S::PostInitInstance(UObject* InObj, FObjectInstancingGraph* InstanceGraph)
{
...
void UPythonGeneratedClass_S::BeginDestroy()
{
	ReleasePythonResources();
	Super::BeginDestroy();
}

void UPythonGeneratedClass_S::ReleasePythonResources()
{
...
void UPythonGeneratedClass_S::UnregisterGeneratedType()
{
	if (PyType)
	{
...
bool UPythonGeneratedClass_S::IsFunctionImplementedInScript(FName InFunctionName) const
{
	UFunction* Function = FindFunctionByName(InFunctionName);
	return Function && Function->GetOuter() && Function->GetOuter()->IsA(UPythonGeneratedClass_S::StaticClass());
}

UPythonGeneratedClass_S* UPythonGeneratedClass_S::GenerateClass(PyTypeObject* InPyType)
{
...
bool UPythonGeneratedClass_S::ReparentDerivedClasses(UPythonGeneratedClass_S* InOldParent, UPythonGeneratedClass_S* InNewParent)
{
	...
	for (UClass* DerivedClass : DerivedClasses)
	{
		...
		if (UPythonGeneratedClass_S* PyDerivedClass = Cast<UPythonGeneratedClass_S>(DerivedClass))
		{
...
UPythonGeneratedClass_S* UPythonGeneratedClass_S::ReparentClass(UPythonGeneratedClass_S* InOldClass, UPythonGeneratedClass_S* InNewParent)
{
	// Builder used to generate the class
	...
	UPythonGeneratedClass_S* NewClass = PythonClassBuilder.Finalize(InOldClass->PyPostInitFunction);
	if (NewClass)
	{
...
DEFINE_FUNCTION(UPythonGeneratedClass_S::CallPythonFunction)
{
	...
	TSharedPtr<PyGenUtil::FFunctionDef> FuncDef;
	{
		...
		const UPythonGeneratedClass_S* This = CastChecked<UPythonGeneratedClass_S>(Func->GetOwnerClass());
