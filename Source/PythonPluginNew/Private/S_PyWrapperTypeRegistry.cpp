...
#include "S_PyWrapperName.h"
#include "S_PyWrapperText.h"
#include "S_PyWrapperArray.h"
#include "S_PyWrapperFixedArray.h"
#include "S_PyWrapperSet.h"
#include "S_PyWrapperMap.h"
#include "S_PyWrapperFieldPath.h"
#include "S_PyConversion.h"
#include "S_PyGIL.h"
#include "S_PyCore.h"
#include "S_PyEditor.h"
#include "S_PyEngine.h"
#include "S_PyFileWriter.h"
...
bool ShouldEllipseParamDefaultValue(const FString& DefaultValue)
{
	...
	const bool bEnsureDefaultValueParses = (PyGenUtil::GetTypeHintingMode() == ETypeHintingMode_S::TypeChecker || IsGeneratingOnlineDoc());
...
void FPyWrapperTypeReinstancer::AddPendingClass(UPythonGeneratedClass_S* OldClass, UPythonGeneratedClass_S* NewClass)
{
	ClassesToReinstance.Emplace(MakeTuple(OldClass, NewClass));
}

void FPyWrapperTypeReinstancer::AddPendingStruct(UPythonGeneratedStruct_S* OldStruct, UPythonGeneratedStruct_S* NewStruct)
{
...
PyTypeObject* FPyWrapperTypeRegistry::GenerateWrappedDelegateType(const UFunction* InDelegateSignature, FGeneratedWrappedTypeReferences& OutGeneratedWrappedTypeReferences, TSet<FName>& OutDirtyModules, const EPyTypeGenerationFlags InGenerationFlags)
{
	...
	{
		PythonCallableForDelegateClass = NewObject<UClass>(GetPythonTypeContainer(), *PythonCallableForDelegateObjectName, RF_Public | RF_Transient);
		UFunction* PythonCallableForDelegateFunc = nullptr;
		{
			...
			FuncDuplicationParams.DestName = UPythonCallableForDelegate_S::GeneratedFuncName;
		...
		PythonCallableForDelegateFunc->SetNativeFunc(&UPythonCallableForDelegate_S::CallPythonNative);
		...
		PythonCallableForDelegateClass->SetSuperStruct(UPythonCallableForDelegate_S::StaticClass());
...
void FPyWrapperTypeRegistry::GenerateStubCodeForWrappedTypes(const EPyOnlineDocsFilterFlags InDocGenFlags) const
{
	...
	auto ProcessWrappedDataArray = [this, &PythonScript, InDocGenFlags](const TMap<FSoftObjectPath, PyTypeObject*>& WrappedData, const TSharedPtr<FPyOnlineDocsSection>& OnlineDocsSection)
	{
...
void FPyWrapperTypeRegistry::GenerateStubCodeForWrappedType(PyTypeObject* PyType, const PyGenUtil::FGeneratedWrappedType* GeneratedTypeData, FPyFileWriter& OutPythonScript, FPyOnlineDocsSection* OutOnlineDocsSection)
{
	...
	const PyGenUtil::EPythonizeFlags PythonizeMethodParamFlags = PyGenUtil::IsTypeHintingEnabled()
		? (PyGenUtil::GetTypeHintingMode() == ETypeHintingMode_S::AutoCompletion
...
{
	FSoftObjectPath& ExistingUnrealTypeName = PythonWrappedTypeNameToUnrealTypeName.FindOrAdd(InPythonTypeName);
	if (ExistingUnrealTypeName.IsNull())
	{
		ExistingUnrealTypeName = InUnrealTypeName;
	}
	else
	{
...
{
	const FSoftObjectPath* ExistingUnrealTypeNamePtr = PythonWrappedTypeNameToUnrealTypeName.Find(InPythonTypeName);
