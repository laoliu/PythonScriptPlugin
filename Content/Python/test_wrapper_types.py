...
class PyTestPythonDefinedObject(unreal.PyTestObject):
	...
	def func_blueprint_implementable(self, value):
		return value * 2
	@unreal.ufunction(override=True)
	def func_blueprint_implementable_packed_getter(self):
		return None
	@unreal.ufunction(override=True)
...
class PyTestDelegateCallableTest:
	...
	def on_struct_delegate(self, item: unreal.PyTestStruct) -> None:
		...
		if type(item) != unreal.PyTestStruct:
			_log_error("Delegate callbable  received the wrong type. Expected a PyTestStruct.")
		self.last_struct_arg = item

	def on_vector_delegate(self, item: unreal.Vector2D) -> None:
		self.callback_count += 1
		if type(item) != unreal.Vector2D:
			_log_error("Delegate callbable received the wrong type. Expected a Vector2D.")
...
def _compare_value(val, other, inverse=False):
	if inverse:
		if val == other:
			_log_error("Value was '{0}' but it wasn't supposed to be!".format(other))
	else:
		if val != other:
			_log_error("Value was '{0}' but '{1}' was expected!".format(val, other))
...
def _compare_property_value(obj, other, prop_name, inverse=False):
	...
	if inverse:
		if obj_value == other_value:
			_log_error("Property '{0}' has the value '{1}' but it wasn't supposed to!".format(prop_name, other_value))
	else:
		if obj_value != other_value:
			_log_error("Property '{0}' has the value '{1}' but '{2}' was expected!".format(prop_name, other_value, obj_value))
...
def _test_set_value(obj, prop_name, value):
	...
	if cur_value != value:
		_log_error("Property '{0}' on '{1}' has the value '{2}' but '{3}' was expected!".format(prop_name, type(obj).__name__, cur_value, value))
...
def _test_object_properties(obj):
	...
	if obj.delegate:
		_log_error("Property 'delegate' on '{0}' is bound when it should be unbound!".format(type(obj).__name__))
	obj.delegate.bind_function(obj, "delegate_property_callback")
	if not obj.delegate:
		_log_error("Property 'delegate' on '{0}' is unbound when it should be bound!".format(type(obj).__name__))
	if obj.delegate:
	...
	if obj.multicast_delegate:
		_log_error("Property 'multicast_delegate' on '{0}' is bound when it should be unbound!".format(type(obj).__name__))
	obj.multicast_delegate.add_function(obj, "multicast_delegate_property_callback")
	if not obj.multicast_delegate:
		_log_error("Property 'multicast_delegate' on '{0}' is unbound when it should be bound!".format(type(obj).__name__))
	if obj.multicast_delegate:
...
def _test_object_method_overrides(obj, expected_bp_val, expected_native_val, expected_native_str):
	...
	_compare_value(obj.func_blueprint_implementable_packed_getter(), None)
	_compare_value(obj.call_func_blueprint_implementable_packed_getter(), None)
...
def _test_struct_extension_methods(s):
	_test_set_value(s, "int", 0)
	s.add_int(10)
	_compare_value(s.int, 10)
	s += 10
	_compare_value(s.int, 20)
	
	_test_set_value(s, "float", 0.0)
	s.add_float(10.0)
	_compare_value(s.float, 10.0)
	s += 10.0
	_compare_value(s.float, 20.0)
	
	_test_set_value(s, "string", "")
	s.add_str("Foo")
	_compare_value(s.string, "Foo")
	s += "Bar"
	_compare_value(s.string, "FooBar")
	
	s.set_bool_mutable()
	_compare_value(s.bool_mutable, True)
	s.clear_bool_mutable()
	_compare_value(s.bool_mutable, False)
	
	s.set_bool_mutable_via_ref()
	_compare_value(s.bool_mutable, True)
	s.clear_bool_mutable_via_ref()
	_compare_value(s.bool_mutable, False)
...
def _test_field_path_type():
	...
	if not unreal.PyTestObject().return_field_path().is_valid():
		_log_error("PyTestObject should return a valid FieldPath")

	# The PyTestObject.field_path is initialized to an existing field in C++.
	if not unreal.PyTestObject().field_path.is_valid():
		_log_error("PyTestObject field path property should be valid")

	fp = unreal.FieldPath()
	if fp.is_valid():
		_log_error("Uninitialize field path should not be valid")

	fp = unreal.FieldPath(valid_fp)
	if not fp.is_valid():
		_log_error("Failed to initialize from a valid path.")

	fp = unreal.FieldPath(invalid_fp)
	if fp.is_valid():
		_log_error("Invalid field path should not be valid")
...
if __name__ == "__main__":
	s = unreal.PyTestStruct()
	_test_common_properties(s)
	_test_struct_extension_methods(s)
	...
	# Test accessing class sparse data via a CDO
	cdo = unreal.PyTestObject.get_default_object()
	_compare_value(cdo.get_editor_property('IntFromSparseData'), 0)
	cdo.set_editor_property('IntFromSparseData', 10)
	_compare_value(cdo.get_editor_property('IntFromSparseData'), 10)
	cdo.set_editor_property('IntFromSparseData', 0)
